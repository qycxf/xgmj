...
func NewHYTable(_tableID int, _robots *Robots, _tableCfg *config.TableCfg) *HYTable {
...
//玩家胡牌
func (t *HYTable) HuPai(_seat *seat.Seat) {
	...
	if huType == HUTYPE_ZIMO {
		...
		//添加这张牌
		logs.Info("HYTableHYTable huCard:%v", huCard)
...
//位置胡牌,计算牌型\积分等
func (t *HYTable) calHuResult(_seat *seat.Seat) {
	...
	if huType == HUTYPE_JIEPAO { //接炮
		...
		if huTypeDetail == HUTYPE_DETAIL_QIANGGANG {
			huCmaj.AddPxId(consts.PXID_HYMJ_QIANGGANG)
	...
	if !huCmaj.IsFristSend {
		huCmaj.AddPxId(consts.PXID_HYMJ_TIANHU)
	}
	if huTypeDetail == HUTYPE_DETAIL_GANG_SHANG_HUA {
		huCmaj.AddPxId(consts.PXID_HYMJ_GANGKAI)
	}
	if huCmaj.IsTing {
		huCmaj.AddPxId(consts.PXID_HYMJ_BAOTING)
	}
	if huCmaj.YinZi {
		huCmaj.AddPxId(consts.PXID_HYMJ_YINGZI)
	}

	//牌型分数-----------------------------------
	for _, v := range huCmaj.HuPxIdArr {
		if v != consts.PXID_HYMJ_PINGHU { //平胡的分数已经算在baseScore中,其他牌型另加分
			pxName := consts.GetHuPxName_HYMJ(v)
			pxScore := consts.GetHuPxScore_HYMJ(v)
			totalScore += pxScore
	...
	if t.Majhong.DSeatID == huSeatID {
		if huType == HUTYPE_ZIMO {
			totalScore += 2
			huCmaj.AddPxScore("庄", 2)
		} else {
			totalScore += 1
			huCmaj.AddPxScore("庄", 1)
		}
	}
	...
	if huCmaj.GetGangCt() > 0 {
		...
		//mg := huCmaj.MianGCt
		zhigang := huCmaj.ZhiGCt
		miangang := huCmaj.MianGCt
		gang := zhigang + miangang
		if gang > 0 {
			totalScore += gang
	...
	if len(huCmaj.HuaPaiArr) > 0 {
		...
		huCmaj.AddPxScore("补花分", len(huCmaj.HuaPaiArr))
	}
...
func (t *HYTable) MakeDSeat() {
	...
	if t.Majhong.GameCt == 1 {
		...
		//之后将由上局庄下一座位坐庄 庄家胡牌继续坐庄，黄庄则庄家下家坐庄，闲家胡牌则谁胡谁做庄
		...
		if t.Majhong.FirstHuSeatID != consts.DefaultIndex {
			...
			logs.Info("tableId:%v---------------->makeDSeat   流局 庄家下一位置:%v", t.TableCfg.TableId, t.Majhong.LastZhuangSeatID)
		}
...
func (t *HYTable) FetcherStartThink(_specialPxId int) {
	//检测自己思考
	...
	if isSelfThink {
		...
		if _cmaj.Check_HY_BuHua(t.TableCfg.FengLing) {
			//系统自动补花
			t.buhuaLogic()
		} else {
			//无须思考,等待出牌
...
func (t *HYTable) ThinkSelfPai(_specialPxId int, isPeng bool) bool {
	...
	//	isbuhua = curtCmaj.Check_HY_BuHua(t.TableCfg.FengLing)
	//}
	//if isbuhua {
	...
	if isHu {
		...
		//		curtCmaj.HuTypeDetail
		//		curtCmaj.AddPxId(_specialPxId)
		//} else if _specialPxId == consts.PXID_HYMJ_TIANHU {
	...
	// 检测报听 --------------------
	isBaoTing := false
	copyPxId := curtCmaj.HuPxIdArr
	t.tableInter.SendCheckTing()
	//logs.Info("curtCmaj.TingCards====================>%v", curtCmaj.TingCards)
	logs.Info("curtCmaj.SendTipArr====================>%v", curtCmaj.SendTipArr)
	if t.TableCfg.BaoTing > 0 && curtCmaj.IsHuOne() && !curtCmaj.IsTing {
		isBaoTing = true
	}
	if isBaoTing {
		curtCmaj.SetOpt(OptTypeTing)
		logs.Info("tableId:%v,[位置%v]可报听", m.TableCfg.TableId, m.CurtSenderIndex)
	}
	curtCmaj.HuPxIdArr = copyPxId
	return isHu || isAnGang || isMianGang || isBaoTing
}

//出牌后其他位置玩家思考
func (t *HYTable) ThinkOtherPai(_tkCard *MCard) bool {
...
// 胡牌检测
func (t *HYTable) CheckHuPai(_seatId int) bool {
	...
	logs.Info("_calHuInfoPxList============------>>>%v", _calHuInfo.PxList)
	logs.Info("_calHuInfo============------>>>%v", _calHuInfo.PxType)
	return t.Check_hupaiRule(_seatId, _calHuInfo)

}
func (t *HYTable) Check_hupaiRule(_seatId int, _calHuInfo *CalHuInfo) bool {
	_cmaj := t.Majhong.CMajArr[_seatId]
	...
	//	if v.IsHYHuaPai(t.TableCfg.FengLing) {
	//		return false
	//	}
	//}
...
//获取胡牌运算对象
func (t *HYTable) GetCalHuObject(_seatId int) *CalHuObject {
...
// 胡牌牌型检测
func (t *HYTable) Check_PxId(_seatId int, _calHuInfo *CalHuInfo) bool {

	huCmaj := t.Majhong.CMajArr[_seatId]
	//七对
	if _calHuInfo.PxType == PXTYPE_7DUI {
		huCmaj.AddPxId(consts.PXID_HYMJ_QIDUI)
		logs.Info("tableId:%v--------Check_PxId---------->七对", t.TableCfg.TableId)
	}
	//单吊
	if huCmaj.Check_DanDiao() {
		huCmaj.AddPxId(consts.PXID_HYMJ_DANDIAO)
		logs.Info("tableId:%v--------Check_PxId---------->单吊", t.TableCfg.TableId)
	}
	//清一色
	if huCmaj.Check_QYS() {
		huCmaj.AddPxId(consts.PXID_HYMJ_QINGYISE)
		logs.Info("tableId:%v--------Check_PxId---------->清一色", t.TableCfg.TableId)
	}
	//对对胡
	if _calHuInfo.PxType != PXTYPE_7DUI && huCmaj.Check_DDHu(_calHuInfo) {
		huCmaj.AddPxId(consts.PXID_HYMJ_DUIDUIHU)
	...
	if yiBanGaoCt > 0 {
		for i := 0; i < yiBanGaoCt; i++ {
			huCmaj.AddPxId(consts.PXID_HYMJ_YIBANGAO)
		}
		logs.Info("tableId:%v--------Check_PxId---------->一般高", t.TableCfg.TableId)
	}
	//海捞
	if t.Majhong.GetRemainPaiCt() < 4 {
		huCmaj.AddPxId(consts.PXID_HYMJ_HAILAO)
		logs.Info("tableId:%v--------Check_PxId---------->海捞", t.TableCfg.TableId)
	}
	//海捞
	if huCmaj.Check_SiGuiYi() > 0 {
		huCmaj.AddPxId(consts.PXID_HYMJ_SIGUIYI)
		logs.Info("tableId:%v--------Check_PxId---------->海捞", t.TableCfg.TableId)
	}
	//无花果
	if t.TableCfg.WuHuaGuo > 0 && len(huCmaj.HuaPaiArr) == 0 {
		huCmaj.AddPxId(consts.PXID_HYMJ_WUHUAGUO)
		logs.Info("tableId:%v--------Check_PxId---------->无花果", t.TableCfg.TableId)
	}
	//一条龙
	if huCmaj.Check_YiTiaoLong() {
		huCmaj.AddPxId(consts.PXID_HYMJ_YITIAOLONG)
		logs.Info("tableId:%v--------Check_PxId---------->一条龙", t.TableCfg.TableId)
	}
...
//// 其他牌型检测
//func (t *HYTable) Check_ExtPxId(_seatId int) bool {
//
//	huCmaj := t.Majhong.CMajArr[_seatId]
//
...
////检测 素牌:没有红中
//func (t *HYTable) Check_SUPAI(_seatId int) bool {
//
//	_cmaj := t.Majhong.CMajArr[_seatId]
...
// 暗杠检测
func (t *HYTable) Check_AnGang(_seatId int) bool {
...
// 最后一个可胡玩家选择取消胡
func (t *HYTable) LastHuThinkerCancer() {
...
//庄家第一次思考
func (t *HYTable) DPlayerFirstThink() {
...
func (t *HYTable) SendCheckTing() {
	...
	logs.Info("tabeId:%v------------------SendCheckTing()-------------->len(_sendTipArr):%v", t.TableCfg.TableId, len(_sendTipArr))
...
// 更新位置当前听牌
func (t *HYTable) UpdateTingCards(_seatId int) {
...
//玩家杠牌
func (t *HYTable) Gang(_seatId int, _data int, _isSave bool) {
...
func (t *HYTable) ThinkQiangGangHu(_gangSeatID int, _tkCard *MCard) bool {
...
//玩家碰牌
func (t *HYTable) Peng(_seatId int) {
	...
	//阜阳碰牌时判断 "热杠"
	//if _cmaj.CanOpt(OptTypeGang) { // 可杠,判断是否可杠这张牌
	//	//hasReGang := false
	//	for _, v := range _cmaj.OptInfo.GangCard {
	//		if _pengCard.EqualByData(v) {
	//			//hasReGang = true
	//			logs.Info("tableId:%v-------------------->位置:%v 碰牌满足热杠条件 GangCard:%v",
	//				t.ID, _seatId, _cmaj.OptInfo.GangCard)
	//
	//			_cmaj.InReGangState = true
	//			_cmaj.ReGangCardData = v
	//			break
	//		}
	//	}
	//}

	_cmaj.DoPeng(_pengCard)
	_cmaj.ResetOptInfo() //清空本位置思考操作

	_cmaj.LastFetchMCard = nil // 碰完牌 将LastFetchCard设置为空,防止发送tableinfo 时被移动,导致显示错误
	t.Majhong.SetCurtSenderIndex(_seatId)
