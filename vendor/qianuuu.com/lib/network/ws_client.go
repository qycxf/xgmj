...
func (client *WSClient) init() {
	...
	if client.ConnNum <= 0 {
		client.ConnNum = 1
		logs.Release("invalid ConnNum, reset to %v", client.ConnNum)
	}
	if client.ConnectInterval <= 0 {
		client.ConnectInterval = 3 * time.Second
		logs.Release("invalid ConnectInterval, reset to %v", client.ConnectInterval)
	}
	if client.PendingWriteNum <= 0 {
		client.PendingWriteNum = 100
		logs.Release("invalid PendingWriteNum, reset to %v", client.PendingWriteNum)
	}
	if client.MaxMsgLen <= 0 {
		client.MaxMsgLen = 4096
		logs.Release("invalid MaxMsgLen, reset to %v", client.MaxMsgLen)
	}
	if client.HandshakeTimeout <= 0 {
		client.HandshakeTimeout = 10 * time.Second
		logs.Release("invalid HandshakeTimeout, reset to %v", client.HandshakeTimeout)
	}
...
func (client *WSClient) dial() *websocket.Conn {
	for {
		...
		logs.Release("connect to %v error: %v", client.Addr, err)
